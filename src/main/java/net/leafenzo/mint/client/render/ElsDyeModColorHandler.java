...
public class ElsDyeModColorHandler {
    public static void registerBlockColorProviders() {
        ...
        for(Block block : ElsDyeModBlocks.HAS_GRASS_COLOR_PROVIDER) {
...
//        );
        ...
        for(Block block : ElsDyeModBlocks.HAS_FOLIAGE_COLOR_PROVIDER) { //Make sure this is done for every block with any color provider!
        ...
        for(Block block : ElsDyeModBlocks.HAS_GRASS_COLOR_PROVIDER) { //Make sure this is done for every block with any color provider!
            ColorProviderRegistry.ITEM.register((stack, tintIndex) -> ColorProviderRegistry.BLOCK.get(((BlockItem) stack.getItem()).getBlock()).getColor(((BlockItem) stack.getItem()).getBlock().getDefaultState(), null, null, tintIndex), block);
        }
    }
}