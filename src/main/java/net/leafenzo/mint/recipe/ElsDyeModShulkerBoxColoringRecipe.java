...
public class ElsDyeModShulkerBoxColoringRecipe extends SpecialCraftingRecipe {
    public ElsDyeModShulkerBoxColoringRecipe(Identifier identifier, CraftingRecipeCategory craftingRecipeCategory) {
        ...
        for(int k = 0; k < recipeInputInventory.size(); ++k) {
            ...
            if (!itemStack.isEmpty()) {
                if (Block.getBlockFromItem(itemStack.getItem()) instanceof ShulkerBoxBlock) {
                    ...
                    if(Arrays.stream(ElsDyeModDyeColor.VALUES).anyMatch((x) -> x == ((DyeItem) itemStack.getItem()).getColor())) {
                        isModdedDye = true;
                    }
    ...
    public ItemStack craft(RecipeInputInventory recipeInputInventory, DynamicRegistryManager dynamicRegistryManager) {
        ...
        ItemStack itemStack3 = ElsDyeModShulkerBoxBlock.getItemStack(ingredientColor);
    ...
    public RecipeSerializer<?> getSerializer() {
        return ElsDyeModRecipeSerializer.MOD_SHULKER_BOX_COLORING_RECIPE;
    }
}
